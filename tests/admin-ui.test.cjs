...
for(const role of ['admin','atendimento','cozinha'])test('panel login and controls for '+role,async()=>{
 const {dom,w,errors}=await setup(role);try{
  ...
  const more=w.document.querySelector('#moreFilters');more.click();assert.equal(w.document.querySelector('#extraFilters').hidden,false);assert.equal(w.document.querySelectorAll('#orders article').length,1);more.click();assert.equal(w.document.querySelector('#extraFilters').hidden,true);
  const manage=w.document.querySelector('#manageDelivery');manage.click();assert.equal(w.document.querySelector('#deliveryManager').hidden,false);manage.click();assert.equal(w.document.querySelector('#deliveryManager').hidden,true);
  if(role==='admin'){w.document.querySelector('[data-tab="appearanceTab"]').click();w.document.querySelector('[data-tab="mediaTab"]').click();assert.ok(w.document.querySelector('#mediaTab').classList.contains('active-panel'));w.document.querySelector('[data-tab="ordersTab"]').click();}
