...
(()=>{
 ...
 const groups=[['Operação',[['ordersTab','Pedidos','orders']]],['Gestão',[['financeTab','Visão geral','chart'],['performanceTab','Desempenho','trend'],['catalogTab','Produtos','box'],['paymentsTab','Pagamentos','card']]],['Configurações',[['appearanceTab','Aparência e imagens','image'],['settingsTab','Configurações','settings'],['securityTab','Acessos','users']]]];
 ...
 const management=actions.querySelector('a[href="gestao.html"]');management.innerHTML=icon('chart')+'<span>Gestão avançada</span>';management.dataset.adminOnly='';document.getElementById('settingsTab').append(management);
 const media=nav.querySelector('[data-tab="mediaTab"]');media.innerHTML=icon('image')+'<span>Gerenciar imagens</span>';document.getElementById('appearanceTab').prepend(media);
 const back=document.createElement('button');back.type='button';back.className='secondary';back.textContent='Voltar para aparência';back.onclick=()=>document.querySelector('[data-tab="appearanceTab"]').click();document.getElementById('mediaTab').prepend(back);
...
(()=>{
 document.querySelectorAll('[data-ui-icon]').forEach(el=>{el.innerHTML=adminIcon(el.dataset.uiIcon);});
 const more=document.getElementById('moreFilters'),extra=document.getElementById('extraFilters');
 more.onclick=()=>{extra.hidden=!extra.hidden;more.setAttribute('aria-expanded',String(!extra.hidden));};
 const manage=document.getElementById('manageDelivery'),manager=document.getElementById('deliveryManager');
 manage.onclick=()=>{manager.hidden=!manager.hidden;manage.setAttribute('aria-expanded',String(!manager.hidden));};
 const menu=document.querySelector('.panel-menu');
 const body=document.body;
 menu.addEventListener('toggle',()=>{body.classList.toggle('menu-visible',menu.open);menu.querySelector('summary').setAttribute('aria-expanded',String(menu.open));});
})();
