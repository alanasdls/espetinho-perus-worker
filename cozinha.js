...
async function api(path,opt={}){const r=await fetch(API+path,{...opt,headers:{...headers(),...(opt.headers||{})},cache:'no-store'});const d=await r.json().catch(()=>({}));if(r.status===401){sessionStorage.removeItem('ep-staff-session');key='';orders=[];render();$('#kitchenLogin').hidden=false;}if(!r.ok)throw new Error(d.erro||`HTTP ${r.status}`);return d}
...
function connectRealtime(){startFallback();}
...
function next(s){return ({recebido:'em_preparo',em_preparo:'pronto_retirada'})[s]||''}
...
<footer>${n?`<button data-status="${n}">${labels[n]}</button>`:''}</footer></article>`}
...
$('#kitchenLoginForm').onsubmit=async e=>{e.preventDefault();const button=e.target.querySelector('button');button.disabled=true;try{
 const r=await fetch(API+'/admin/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:$('#kitchenEmail').value,password:$('#kitchenKey').value})});const d=await r.json();if(!r.ok)throw Error(d.erro||'Não foi possível entrar.');key=d.token;sessionStorage.setItem('ep-staff-session',key);$('#kitchenKey').value='';$('#kitchenLogin').hidden=true;await load();connectRealtime();
}catch(err){$('#kitchenError').textContent=err.message;}finally{button.disabled=false;}};
$('#kitchenLogout').onclick=async()=>{try{if(key)await api('/admin/auth/logout',{method:'POST'});}finally{sessionStorage.removeItem('ep-staff-session');location.reload();}};
